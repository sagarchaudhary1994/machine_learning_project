...
class DataIngestion:
    ...
    def download_house_data(self) -> str:
        try:
            # extracting remote url to download url
    ...
    def split_data_as_train_test(self) -> DataIngestionArtifact:
        try:
            ...
            # get the file name from raw_data_dir
            files_list = os.listdir(path=raw_data_dir)
            file_Name: str = None
            if len(files_list) > 0:
            ...
            # get the extension of housing_file_path
            file_extension = pathlib.Path(file_Name).suffix
            housing_data_frame: pd.DataFrame = pd.DataFrame()
            if file_extension == ".csv":
                ...
                for train_index, test_index in split.split(housing_data_frame, housing_data_frame["income_cat"]):
                    strat_train_set = housing_data_frame.loc[train_index].drop(columns=[
                                                                               "income_cat"])
                    strat_test_set = housing_data_frame.loc[test_index].drop(columns=[
                                                                             "income_cat"])
